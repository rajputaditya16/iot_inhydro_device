...
def offline_buffer_worker():
    while True:
        ...
        if batch_to_send:
            ...
            # Strip 'entry_no' before sending to ThingSpeak to prevent 400 Bad Request
            clean_batch = [{k: v for k, v in item.items() if k != 'entry_no'} for item in batch_to_send]
            success = send_bulk_update(channel_id, write_key, clean_batch)
            if success:
                with offline_lock:
                    if os.path.exists(OFFLINE_FILE):
                        with open(OFFLINE_FILE, "r") as f:
                        ...
                        if len(buffer) >= len(batch_to_send):
                            ...
                            # Re-index entry numbers for remaining records
                            for idx, item in enumerate(buffer):
                                item["entry_no"] = idx + 1
                            if not buffer:
                                with open(OFFLINE_FILE, "w") as f:
                ...
                time.sleep(16)  # Strictly 16 seconds delay to respect ThingSpeak 15s rate limit!
            else:
                time.sleep(16)  # Backoff if failed
        else:
            time.sleep(5)  # Idle sleep when no backlog exists
