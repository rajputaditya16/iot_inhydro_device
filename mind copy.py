...
CLOUD_BROKER = "147.93.106.142"
MQTT_PORT = 1883
CLOUD_USER = "Inhydro@5598"
CLOUD_PASS = "MGPL@5598"
...
# Connect to Cloud Broker (Mosquitto VPS)
...
try:
    ...
    print("Successfully connected to Mosquitto VPS Cloud Broker.")
except Exception as e:
...
def check_timeframes_conflict(time_frames):
    ...
    for idx, tf in enumerate(time_frames):
        ...
        # Check for overlap without any 5-minute buffer delay
        if start_mins < end_mins:
    ...
    for m in range(1440):
        if minute_map[m] > 1:
            h = m // 60
            mi = m % 60
            return True, f"Timeframes overlap detected around {h:02d}:{mi:02d}."
...
def create_edit_cell(parent, label_text, val_text, on_edit_click):
    ...
    lbl = tk.Label(cell, text=label_text, fg=MUTED_TEXT, bg=CARD_BG, font=("Helvetica", 12, "bold"), width=16, anchor="w")
...
class MainControllerApp:
    def __init__(self, root):
        ...
        # Threading for non-blocking Wi-Fi status checking
        import threading
        self.wifi_connected = False
        self.wifi_ssid = "Disconnected"
        self.stop_wifi_thread = False
        
        def bg_check_wifi():
            import subprocess
            while not self.stop_wifi_thread:
                try:
                    # Run 'iwgetid -r' to get the SSID. This will succeed and return the SSID if connected.
        ...
        self.wifi_thread = threading.Thread(target=bg_check_wifi, daemon=True)
    ...
    def build_light_rows(self):
        ...
        for idx in range(1, 9):
            ...
            lbl_dot = tk.Label(info_frame, text="●", fg="#dc2626", bg=CARD_BG, font=("Helvetica", 16), cursor="hand2")
            lbl_dot.pack(side="left", padx=(0, 8))
            
            # Click handler to open password dialog for this specific channel
            def make_click_handler(channel_id):
                return lambda event=None: self.open_password_dialog(channel_id)
                
            click_handler = make_click_handler(idx)
            ...
            # Bind the left click / tap event to make the whole card touchable
            card.bind("<Button-1>", click_handler)
            info_frame.bind("<Button-1>", click_handler)
            lbl_dot.bind("<Button-1>", click_handler)
            lbl_intensity.bind("<Button-1>", click_handler)
            lbl_frame.bind("<Button-1>", click_handler)
    ...
    def update_clock_and_schedule(self):
        ...
        # Update Wi-Fi status label dynamically
        try:
            if hasattr(self, 'lbl_local_status') and self.lbl_local_status.winfo_exists():
                if self.wifi_connected:
                    self.lbl_local_status.config(
                        text=f"● Wi-Fi: {self.wifi_ssid}",
                        fg=ACCENT_GREEN
                    )
                else:
                    self.lbl_local_status.config(
            ...
            print(f"Error updating wifi status label: {e}")
    ...
    def open_password_dialog(self, target_channel_id=1):
        ...
        def kp_confirm():
            ...
            if password_entered == correct_password:
                win.destroy()
                self.open_setpoints_window(target_channel_id)
            else:
    ...
    def open_setpoints_window(self, default_channel_id=1):
        try:
            self._open_setpoints_window(default_channel_id)
        except Exception as e:
            import traceback
            err_msg = traceback.format_exc()
            messagebox.showerror("Error", f"Failed to load Setpoints Window:\n{err_msg}", parent=self.root)

    def _open_setpoints_window(self, default_channel_id=1):
        ...
        header.pack(fill="x", padx=30, pady=(2, 1))
        ...
        lbl_title.pack(side="left", pady=1)
        ...
        top_bar.pack(fill="x", padx=30, pady=(2, 4))
        ...
        def toggle_custom_dropdown():
            nonlocal dropdown_open
            if dropdown_open:
                dropdown_menu_frame.place_forget()
                dropdown_open = False
            else:
                for w in dropdown_menu_frame.winfo_children():
                    w.destroy()
                
                for i in range(1, 9):
                    ch_name = local_config_copy['lights'][i-1]['name']
                    btn_text = f"Ch {i} - {ch_name}"
                    ...
                    def make_select_handler(channel_idx):
                        return lambda: select_custom_channel(channel_idx)
                        
                    btn.config(command=make_select_handler(i))
                    btn.pack(fill="x")
                    
                    if i < 8:
                        sep = tk.Frame(dropdown_menu_frame, bg="#cbd5e1", height=1)
                        sep.pack(fill="x")
                        
                dropdown_menu_frame.place(
                    in_=combo_btn,
                    relx=0.0,
                    rely=1.0,
                    y=2,
                ...
                dropdown_menu_frame.lift()
                dropdown_open = True

        def select_custom_channel(channel_idx):
            nonlocal current_channel_id, dropdown_open
            current_channel_id = channel_idx
            ch_name = local_config_copy['lights'][channel_idx - 1]['name']
            combo_btn.config(text=f"Ch {channel_idx} - {ch_name}  ▼")
            lbl_channel_name.config(text=ch_name)
            rebuild_timeframe_grid()
            dropdown_menu_frame.place_forget()
            dropdown_open = False

        # Elegant Custom Dropdown Button
        combo_btn = tk.Button(
            ch_selector_frame,
            text=dropdown_options[default_channel_id - 1] + "  ▼",
            ...
            pady=10,
            cursor="hand2",
            command=toggle_custom_dropdown
        )
        combo_btn.pack(side="left")
        ...
        def on_click_outside(event):
            ...
            if dropdown_open:
                ...
                in_dropdown = False
                while w:
                    if w == dropdown_menu_frame or w == combo_btn:
                        in_dropdown = True
                        break
                    try:
                        w = w.master
        ...
        set_win.bind("<Button-1>", on_click_outside)
        ...
        lbl_channel_name = tk.Label(name_cell_frame, text=local_config_copy['lights'][default_channel_id - 1]['name'], fg=TEXT_COLOR, bg=CARD_BG, font=("Helvetica", 13, "bold"))
            ...
            def on_keypad_submit(new_val):
                ...
                if frame_idx is None:
                    if field == "password":
                        ...
                        combo_btn.config(text=f"Ch {light_id} - {new_val}  ▼")
                else:
                    frame = light["time_frames"][frame_idx]
                    if field == "name":
                        if not new_val:
                        ...
                        old_time = frame[field]
                        temp_time = f"{h:02d}:{m:02d}"
                        frame[field] = temp_time
                        
                        # Instant validation to prevent overlap or invalid times from being entered
                        has_conflict, error_msg = check_timeframes_conflict(light["time_frames"])
                        if has_conflict:
                            frame[field] = old_time
                            messagebox.showerror("Time Conflict Error", f"Cannot set time: {error_msg}", parent=set_win)
                            return
                            
                        val_lbl.config(text=temp_time, width=len(temp_time) + 2)
        ...
        tf_container.pack(fill="x", expand=False, padx=30, pady=(2, 4))
        ...
        def build_timeframe_card(parent, light_id, f_idx, row, col):
            ...
            f_frame.grid(row=row, column=col, padx=15, pady=5, sticky="nsew")
            ...
            # Horizontal Scale Slider under Intensity - Made thick and easy to hit on touchscreens
            slider = tk.Scale(
                f_frame,
                from_=0,
                to=100,
                ...
                bd=0,
                width=24,                  # Thick trough (height of track)
                sliderlength=42,            # Wide slider handle
                showvalue=False
            )
            slider.set(frame["intensity"])
            slider.pack(fill="x", pady=(2, 4), padx=10)
            
            # Smooth touch control: jump to the exact touched coordinate immediately
            def jump_to_click(event):
                if slider.identify(event.x, event.y) != "slider":
                    w = slider.winfo_width()
                    slen = int(slider.cget("sliderlength"))
                    margin = slen / 2
                    effective_w = w - slen
                    if effective_w > 0:
                        x = event.x - margin
                        x = max(0, min(x, effective_w))
                        val = (x / effective_w) * 100
                        slider.set(int(val))
        ...
        def rebuild_timeframe_grid():
            ...
            # Force columns to stretch wider for beautiful fullscreen view
            cards_inner.columnconfigure(0, weight=1, minsize=480)
            cards_inner.columnconfigure(1, weight=1, minsize=480)
        ...
        set_footer.pack(fill="x", side="bottom", padx=30, pady=(2, 4))
