...
def control_room(room, data):
    ...
    if soil:
        ...
        if do_val is not None:
            if not st.get("do_active", False) and do_val < sp.get("DO MIN", 5.0):
                st["do_active"] = True
                relay_on(ch["aerator"])
                warnings.append("⚠ DO LOW — AERATOR ON")
                print(f"Room{room} Aerator ON — DO: {do_val} mg/L < {sp.get('DO MIN', 5.0)} mg/L")
            elif st.get("do_active", False) and do_val >= sp.get("DO MAX", 8.0):
                st["do_active"] = False
                relay_off(ch["aerator"])
                print(f"Room{room} Aerator OFF — DO: {do_val} mg/L >= {sp.get('DO MAX', 8.0)} mg/L")
        else:
            if st.get("do_active", False) or relay_is_on(ch["aerator"]):
                st["do_active"] = False
                relay_off(ch["aerator"])
    else:
        if st["ec_active"] or st["ph_active"] or st["chiller_active"] or st.get("do_active", False) or relay_is_on(ch["ec1"]) or relay_is_on(ch["ph"]) or relay_is_on(ch["chiller"]) or relay_is_on(ch["aerator"]):
            relay_off(ch["ec1"])
            relay_off(ch["ec2"])
            relay_off(ch["ph"])
            relay_off(ch["chiller"])
...
def publish_telemetry():
    ...
    if current_time - last_cloud_publish_time < 1:
        return
...
def update_ui():
    ...
    soil_data = {
        ...
        "do": latest_raw.get("do")
    } if any(latest_raw.get(k) is not None for k in ["water_temp", "moisture", "ec", "ph", "do"]) else None
