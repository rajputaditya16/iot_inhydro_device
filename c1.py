...
def publish_telemetry(d1, d2):
    ...
    if current_time - last_cloud_publish_time < 25:
        return
    if not mqtt_client or not MQTT_TOPIC: return
    try:
        def arr(a, b): return json.dumps([a, b])
        s1 = d1.get("soil"); s2 = d2.get("soil")
        r1 = d1.get("room"); r2 = d2.get("room")
        payload = (
            f"field1={arr(_v(s1.get('soil_temp') if s1 else None), _v(s2.get('soil_temp') if s2 else None))}"
            f"&field2={arr(_v(s1.get('moisture')  if s1 else None), _v(s2.get('moisture')  if s2 else None))}"
            f"&field3={arr(_v(s1.get('ec')        if s1 else None), _v(s2.get('ec')        if s2 else None))}"
            f"&field4={arr(_v(s1.get('ph')        if s1 else None), _v(s2.get('ph')        if s2 else None))}"
            f"&field5={arr(_v(r1.get('room_temp') if r1 else None), _v(r2.get('room_temp') if r2 else None))}"
            f"&field6={arr(_v(r1.get('room_humi') if r1 else None), _v(r2.get('room_humi') if r2 else None))}"
            f"&field7={arr(_v(d1.get('orp')), _v(d2.get('orp')))}"
            f"&field8={arr(_v(d1.get('co2')), _v(d2.get('co2')))}"
        )
...
_sensor_cache = {}  # Cache working modbus address, baudrate, and brand to ensure sub-millisecond future reads

def read_soil(inst, label):
    if not inst: return None
    
    orig_timeout = inst.serial.timeout
    inst.serial.timeout = 1.0  # Generous timeout for pure real-time
    
    # Try both baud rates in pure real-time, zero cache!
    for baud in [9600, 4800]:
        if inst.serial.baudrate != baud:
            inst.serial.baudrate = baud
            
        for fc in [3, 4]:
            # Priority 1: True EC (0x0015) and True pH (0x0006)
            try:
                inst.serial.reset_input_buffer()
                ec = inst.read_register(0x0015, 0, functioncode=fc)
                
                time.sleep(0.08)
                inst.serial.reset_input_buffer()
                ph_raw = inst.read_register(0x0006, 0, functioncode=fc)
                
                ph = ph_raw / 100.0 if ph_raw >= 150 else ph_raw / 10.0
                if ec >= 0 and ph >= 0:
            ...
            # Priority 2 (Fallback): Alternative EC (0x0002) and pH (0x0003) 
            # (Only if Priority 1 fails, to avoid reading Nitrogen/Phosphorus by mistake)
            try:
                inst.serial.reset_input_buffer()
                ec = inst.read_register(0x0002, 0, functioncode=fc)
                
                time.sleep(0.08)
                inst.serial.reset_input_buffer()
                ph_raw = inst.read_register(0x0003, 0, functioncode=fc)
...
def read_md02(inst, label):
    ...
    # 1. Cached sub-millisecond read
    cached = _sensor_cache.get(label)
    if cached:
        inst.address = cached["sid"]
        try:
            inst.serial.reset_input_buffer()
            if cached["brand"] == "fun4":
                rt = inst.read_register(1, 1, functioncode=4, signed=True)
                rh = inst.read_register(2, 1, functioncode=4)
            else:
                rt = inst.read_register(1, 1, functioncode=3, signed=True)
                rh = inst.read_register(2, 1, functioncode=3)
            return {"room_temp": rt, "room_humi": rh}
        except Exception:
            _sensor_cache.pop(label, None)
    ...
    for sid in [1, 2]:
        ...
        try:
            ...
            try:
                rt = inst.read_register(1, 1, functioncode=4, signed=True)
                rh = inst.read_register(2, 1, functioncode=4)
                _sensor_cache[label] = {"sid": sid, "brand": "fun4"}
                inst.serial.timeout = orig_timeout
                return {"room_temp": rt, "room_humi": rh}
            except:
                rt = inst.read_register(1, 1, functioncode=3, signed=True)
                rh = inst.read_register(2, 1, functioncode=3)
                _sensor_cache[label] = {"sid": sid, "brand": "fun3"}
                inst.serial.timeout = orig_timeout
                return {"room_temp": rt, "room_humi": rh}
        except:
            continue
...
timer_state = {
    1: [{"state": "OFF", "last": 0.0}, {"state": "OFF", "last": 0.0}, {"state": "OFF", "last": 0.0}],
    2: [{"state": "OFF", "last": 0.0}, {"state": "OFF", "last": 0.0}, {"state": "OFF", "last": 0.0}],
}

TIMER_CHANNELS = {
    1: [R1_CH_TMR1, R1_CH_TMR2, R1_CH_TMR3],
    2: [R2_CH_TMR1, R2_CH_TMR2, R2_CH_TMR3],
}
...
def make_home_box(room):
    ...
    keys = ["ec","ph","room_temp","room_humi","orp","co2"]
    labels_map = {}
    display = {
...
def build_room_screen(room):
    ...
    for i in range(3):
        ...
        lbl_tname = tk.Label(col_R, text=setpoints[room].get(tname_key, f"TIMER {i+1}"),
                             font=("Arial",12,"bold"), fg="#1565c0", bg="#e0e0e0")
        lbl_tname.pack(pady=(4,1))
        ...
        for sub_key, sub_lbl in [
            ...
            f = tk.Frame(col_R, bg="white"); f.pack(fill="x", pady=1)
            tk.Label(f, text=sub_lbl, font=TINY, fg="#666",
                     bg="white", width=8, anchor="w").pack(side="left")
            lbl = tk.Label(f, text="---", font=TINY, fg="#333", bg="white", anchor="e")
            lbl.pack(side="right")
...
def build_setpoint_screen(room):
    ...
    def create_row(key):
        ...
        cell.grid(row=row_i, column=col_i, padx=8, pady=2)
        tk.Label(cell, text=key, font=SML, fg="#333",
