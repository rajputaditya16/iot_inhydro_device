...
R2_PORT_SOIL = "/dev/serial/by-path/pci-0000:0000:14.0-usb-0:3:1.0-port0"
...
R3_PORT_MD02_2 = "/dev/serial/by-path/pci-0000:00:14.0-usb-0:4:1.0-port0"
...
R2_soil = None  # Room 2 EC (ID 3) & pH (ID 1) read dynamically via read_water_meters_r2
...
def read_water_meters_r2(preferred_port):
    """
    Reads Room 2 Water Meters: EC (Slave ID 3) and pH (Slave ID 1) on a single serial path per check_water_meters.py.
    """
    ports_to_try = []
    if preferred_port and os.path.exists(preferred_port):
        ports_to_try.append(preferred_port)

    # Fallback to scan available ports if preferred_port is not available/responsive
    by_path_dir = "/dev/serial/by-path"
    if os.path.exists(by_path_dir):
        for f in os.listdir(by_path_dir):
            fp = os.path.join(by_path_dir, f)
            if fp not in ports_to_try:
                ports_to_try.append(fp)
    import glob
    for tp in sorted(glob.glob("/dev/ttyUSB*") + glob.glob("/dev/ttyACM*")):
        if tp not in ports_to_try and os.path.realpath(tp) not in [os.path.realpath(p) for p in ports_to_try]:
            ports_to_try.append(tp)
    ...
    for port in ports_to_try:
        inst = None
        try:
            # Single serial port opening for both EC and pH meters
            inst = minimalmodbus.Instrument(port, 3)
            inst.serial.baudrate = 9600
            inst.serial.timeout = 0.25
            inst.mode = minimalmodbus.MODE_RTU
            inst.clear_buffers_before_each_transaction = True

            # 1. Read EC (Slave ID 3) on this path
            inst.address = 3
            for fc in [3, 4]:
                for reg in [3, 0, 1]:
                    try:
                        ...
                        if val is not None and val > 0:
                            v = val * 1000 if val <= 10 else val
                            raw_ec = int(round(v / 0.85))
                            break
                    except Exception:
                        pass
            ...
            for fc in [3, 4]:
                for reg in [0, 2, 1, 3]:
                    try:
                        val = inst.read_register(reg, 0, functioncode=fc)
                        if val is not None and 300 <= val <= 1100:
                            ph_val = round(val / 100.0, 2)
                            break
                    except Exception:
                        pass
            ...
            if raw_ec is not None or ph_val is not None:
                break
        except Exception:
            pass
        finally:
    ...
    if raw_ec is None and ph_val is None:
        return None
...
def read_all_sensors(room):
    """Read Soil/Water, Room, ORP, and CO2 sensors as requested."""
    if room == 1:
        return {
        ...
        return {
            "soil": read_water_meters_r2(R2_PORT_SOIL),
...
def build_room_screen(room):
    ...
    tk.Button(foot, text="RESTART", font=MED, width=10,
              command=lambda: restart_program()).pack(side="left", padx=8, pady=8)
    ...
    if room == 3:
        ...
        if soil:
            ...
            if soil.get('ec') is not None:
                ...
                tds_ec = disp_ec * 500
                sv("ec", f"{disp_ec:.3f} mS/cm ({tds_ec:.0f} ppm)")
            else:
