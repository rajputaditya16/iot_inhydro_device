...
const CustomTooltip = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 shadow-xl space-y-1">
        <p className="text-xs text-slate-400">{label}</p>
        {payload.map((p, idx) => (
          <p key={idx} className="text-sm font-semibold text-white">
            <span style={{ color: p.color || p.stroke }}>{p.name || 'Value'}:</span> {p.value}
          </p>
        ))}
      </div>
    );
