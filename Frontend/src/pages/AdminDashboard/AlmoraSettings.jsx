...
const AlmoraSettings = () => {
  ...
  useEffect(() => {
    ...
    const handleIncomingPacket = (topic, messageData) => {
      if (topic === `inhydro/${deviceRoot}/telemetry/live`) {
        try {
          const incomingLiveData = typeof messageData === 'string' ? JSON.parse(messageData) : messageData;
  ...
  const handleSave = async () => {
    ...
    const numericFields = ['EC MIN', 'EC MAX', 'PH LOW', 'PH HIGH'];
    ...
    // 1. Try direct WebSocket MQTT Publish
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Manage Hydroponics EC, pH & Private Broker Telemetry Setpoints</p>
        </div>
